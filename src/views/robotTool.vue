                  ...
                  <el-col :span="24">
                    <el-card shadow="never">
                      ...
                      <div v-for="log in logs" :key="log.id" class="text item">
                        {{log.account}}:{{log.count}}{{log.log}},时间：{{log.diff}}
                      </div>
                    </el-card>
                  </el-col>
                </el-row>
...
Date.prototype.Format = function (formatStr) {
  ...
  str = str.replace(/xx|XX/, this.getMilliseconds());
  ...
  data() {
    return {
      ...
      rules: {
        ...
        advance: [{ required: true, message: '提前时间不能为空' }, { type: 'number', message: '提前时间必须为数字值' }],
        step: [{ required: true, message: '间隔时间不能为空' }, { type: 'number', message: '间隔时间必须为数字值' }],
        count: [{ required: true, message: '领取次数不能为空' }, { type: 'number', message: '领取次数必须为数字值' }]
      },
      accountForm: {
        name: "",
        value: ""
  ...
  methods: {
    ...
    async init() {
      try {
        ...
        let h = this.ruleForm.startTime.substr(0, this.ruleForm.startTime.indexOf(':'));
        let m = this.ruleForm.startTime.substr(this.ruleForm.startTime.indexOf(':') + 1);
        var starttime = new Date(parseInt(res.currentTime2));//服务器时间
        var hours = starttime.setHours(h);
        var min = starttime.setMinutes(m);
        var sec = starttime.setSeconds(0);
        var count = 0;
        var interval = 50;
        let preStartUp = false;//预先启动，降低冷启动、并发实例及业务代码初始化引起的耗时
        //var misec = starttime.setMilliseconds(ms || 0);
        starttime = new Date(new Date(starttime).getTime() + this.ruleForm.advance);
        ...
        let _interval = function () {
          ...
          console.log('%c' + parseInt((starttimes) - parseInt(nowtimes)) / 1000 + '秒后开始,时间差:' + difference + 'ms', 'color: #7ED0A7;')
          _this.showInfo = parseInt((starttimes) - parseInt(nowtimes)) / 1000 + '秒后开始,时间差:' + difference + 'ms';
    ...
    getJdCoupon(el, index, count) {
      ...
      if (!el.disabled) {
        jd.getCoupon({
          ...
          let diff = moment(new Date().getTime() - parseInt(_this.currentTime)).format("HH:mm:ss:SSS");
          _this.logs.push({ 'account': 'No.' + (index + 1), 'count': '第' + count + '次', 'log': JSON.stringify(res), id: index + "" + count + "" + new Date().getTime(), diff: diff })
        }).catch((error) => {
          let diff = moment(new Date().getTime() - parseInt(_this.currentTime)).format("HH:mm:ss:SSS");
          _this.logs.push({ 'account': 'No.' + (index + 1), 'count': '第' + count + '次', 'log': JSON.stringify(error), id: index + "" + count + "" + new Date().getTime(), diff: diff })
        })
      }
    },
    clearLog() {
